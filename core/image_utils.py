...
# 强兼容策略：允许加载超大图，仅忽略 Pillow 的 DecompressionBombWarning。
...
if _HAS_ICC_SUPPORT:
    try:
        _SRGB_PROFILE_PATH = ImageCms.createProfile("sRGB")
    except Exception:
...
def _apply_icc_to_srgb(im: Image.Image, path: str) -> Image.Image:
    ...
    try:
        src_profile = ImageCms.ImageCmsProfile(icc_data)
        dst_profile = ImageCms.ImageCmsProfile(_SRGB_PROFILE_PATH)
        ...
        if _has_alpha_channel(im):
            ...
            converted = ImageCms.profileToProfile(im, src_profile, dst_profile, outputMode="RGB")
...
def _to_display_l_channel(im: Image.Image) -> Image.Image:
    ...
    if mode in {"I", "F", "I;16", "I;16L", "I;16B", "I;16N"}:
        ...
        # 注意：I 模式（32位整数）不支持 point(lambda)，需用 ImageMath 或 numpy
        try:
            from PIL import ImageMath
            result = ImageMath.eval(
                "convert(max(0, min(255, im * scale + offset)), 'L')",
                im=i_img, scale=float(scale), offset=float(offset)
            )
            # ImageMath.eval 返回的可能是 ImageMath 客体，需确保转为 Image
            if hasattr(result, "convert"):
                return result.convert("L")
            return result
        except Exception as e:
            _dbg(f"image_math_failed err={e}, falling back to numpy/manual")
            try:
                import numpy as np
                ...
                # 最终回退：逐像素处理（较慢但可靠）
                _dbg("numpy not available, using manual pixel processing")
                pixels = list(i_img.getdata())
...
def _is_frame_blank(im: Image.Image, threshold: float = 0.99) -> bool:
    ...
    try:
        ...
        # 如果最大值和最小值都接近 0（全黑）或都接近 255（全白），则视为空白
        ...
        black_count = sum(1 for p in pixels if p <= 5)
